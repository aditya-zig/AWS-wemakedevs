...
const messages=['Resolving repository and commit…','Planning applicable specialist workers…','Launching isolated workers…','Running scoped checks…','Collecting executed evidence…','Classifying uncertainty…','Cross-checking worker reports…','Preparing the evidence report…'];
...
function actions(){$$('[data-audit-trigger]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();openDrawer()}));$$('[data-nav="login"].primary').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();openDrawer()}));$('#closeDrawer')?.addEventListener('click',closeDrawer);$('#backdrop')?.addEventListener('click',closeDrawer);$('#drawerRun')?.addEventListener('click',()=>runFlagshipAudit({drawer:false}));$('#watchAudit')?.addEventListener('click',()=>{$('#live-audit')?.scrollIntoView({behavior:reducedMotion?'auto':'smooth'});setTimeout(()=>runFlagshipAudit({drawer:false}),reducedMotion?0:420)});$('#runAuditFromConsole')?.addEventListener('click',()=>runFlagshipAudit({drawer:false}));$('#runDeepAudit')?.addEventListener('click',()=>runFlagshipAudit({drawer:false}));$('#createPr')?.addEventListener('click',()=>toast('PR stays locked until the real Repair + independent verification pipeline returns a PR-ready package.'));$('#payDemo')?.addEventListener('click',()=>toast('Run Deep Audit to collect executed evidence before showing a verdict.'));addEventListener('keydown',e=>{if(e.key==='Escape')closeDrawer()})}
